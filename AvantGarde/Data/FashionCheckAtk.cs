...
public class FashionCheckAtk
{
    ...
    private static bool IsLeftSide(uint itemId)
    {
...
Score Calculation:
    ...
    An active hint drops the score down to 2 points. The stamp "upgrades" for every 1/2 points gained.
    Points can be gained either by choosing the correct item for the hint or the correct dye for the slot.
    Barring extremely rare circumstances (unknown whether to still exist) all correct items grant at leaast +6/8p.
    For each slot with an active hint there exists an item (very rarely multiple) which grants an additional point (+7/9p). Not persistent?
